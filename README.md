...
protobuf的反射机制？
    见[https://editor.csdn.net/md/?articleId=136565050&spm=1001.2014.3001.9457](https://editor.csdn.net/md/?articleId=136565050&spm=1001.2014.3001.9457)
...
zk和Nginx有没有做集群部署？
    ...
    zk集群也是CP强一致性模型，采用的是ZAB协议，大致原理：一个写请求来了，leader把他当成一个事务提案，原子广播给所有follower，若半数以上同意，就执行这条请求。此过程保证所有写请求在所有节点上的执行顺序是一致的。
    所以ZAB模型是多数节点共同决定写请求

    zk部署了三台，当有zk客户端连接，他会自动负载均衡，选择连到哪个zk节点
    zk也可以实现负载均衡转发：
    直接把rpc节点的ip+port注册进zk,然后由zk来选择一个转发也是可以的
    或者就像我这样，zk找到nginx,直接把请求转给nginx，由nginx做转发

分布式锁的实现？
    zk:临时节点，客户端注册一个临时节点到zk的指定锁路径，zk分配一个序号给他，若当前它的序号最小，就获得锁，若不是最小，就拿不到锁，当小序号节点被删除后,zk会通知客户端，它再次检查序号大小
    redis:SETNX, 设置一个键的值若键不存在就成功设置，获得锁。
...
对muduo库的了解？
    ...
    我们使用它也很简单，写好2个回调的逻辑即可，连接回调(但连接断开关闭文件描述符)、读写回调(对于我们这里：先反序列数据，然后调RPC方法，序列化数据，发回响应)
...
rpc节点间有相互调用吗？
有，我特地加了个业务让他去调，就是说若客户端对mysql的访问是对表结构的增删改，我们update接口内部会去调另一个调另一个rpc节点的方法，由他去做这个业务，内部实现就是把rpc节点当客户端来发起调用。但是我这个完全是为了调用而调用。
